...
def SafeMultiExperiment(args):
    try:
        print(" Starting Simulation %d, Entropy 0.%d, Predator %d, Visual Range %s"%(args[3], args[0][0], args[1][0], str(args[2])))
...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        planningDir = os.path.dirname(os.getcwd())
        ...
        for simulationInd in range(ExperimentParams.NumRuns):
            for occlusionInd in range(len(ExperimentParams.EntropyLevels)):
                for predatorInd in range(ExperimentParams.NumPredators):
                    try:
                        if np.isnan(predatorList[predatorInd, simulationInd, occlusionInd]):
                            continue
                    except TypeError:
                        pass
                    ...
                    if visualRange:
                        for visrange in visualRange:
                            tasks.append([(occlusionInd, occlusionList[simulationInd][occlusionInd]),
                                          (predatorInd, predatorList[predatorInd, simulationInd, occlusionInd]), visrange,
                                          simulationInd, planningDir])
                    tasks.append([(occlusionInd, occlusionList[simulationInd][occlusionInd]),
                            (predatorInd, predatorList[predatorInd, simulationInd, occlusionInd]), None, simulationInd, planningDir])
    #SafeMultiExperiment(tasks[0])
        task_index = 0
        numWorkers = 1# size - 1
        closedWorkers = 0
