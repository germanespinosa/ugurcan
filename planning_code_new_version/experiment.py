...
#                break
    ...
    def DiscountedReturn(self, occlusions, predatorHome, knowledge,
        ...
        if visualRange is None:
            summaryDirectory = simulationDirectory + '/Occlusion_%d/Predator_%d' % (occlusionInd, predatorInd)
        else:
            summaryDirectory = simulationDirectory + '/Occlusion_%d/Predator_%d/VisualRange_%d' % \
                          (occlusionInd, predatorInd, visualRange)
        Path(summaryDirectory).mkdir(parents=True, exist_ok=True)

        summaryFile = summaryDirectory + '/Summary.csv'
        summary = {'Depth 100': [],
                   'Depth 1000': []}
        summary_df = pd.read_csv(summaryFile, header=0)

        tempDepths = ExperimentParams.Depth
        for depth in ExperimentParams.Depth[:-1]:
            if summary_df['Depth %d' % (depth)].last_valid_index() == ExperimentParams.MaxDoubles - 1:
                summary['Depth %d' % (depth)] = summary_df['Depth %d' % (depth)].values
                tempDepths.remove(depth)
                continue
        ExperimentParams.Depth = tempDepths
        for depth in ExperimentParams.Depth:
            if visualRange is None:
                directory = simulationDirectory + '/Occlusion_%d/Predator_%d/Depth_5000' % (occlusionInd, predatorInd)
                Path(directory).mkdir(parents=True, exist_ok=True)
            else:
                directory = simulationDirectory + '/Occlusion_%d/Predator_%d/VisualRange_%d/Depth_5000' % (
                occlusionInd, predatorInd, visualRange)
                Path(directory).mkdir(parents=True, exist_ok=True)

            for trial in range(ExperimentParams.MinDoubles, ExperimentParams.MaxDoubles):
                print('depth %d, trial # %d'%(depth, trial))
                if depth == 5000:
