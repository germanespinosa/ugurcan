...
class Game(Simulator):
    def __init__(self, xsize, ysize, visualrange=1, occlusions=[], verbose=False):
        ...
        #invalidPredatorLocations = [self.GoalPos, self.AgentHome] + visualArea + occlusions
        allPredatorLocations = [COORD(x, y) for x in range(xsize) for y in range(ysize)]
        invalidPredatorLocations = [self.GoalPos, self.AgentHome] + occlusions
        for coord in visualArea:
            if self.Grid.VisualRay((self.AgentHome).Copy(), coord.Copy(), self.Occlusions)[0]:
                invalidPredatorLocations.append(coord)
    ...
    def CreateRandomStartState(self):
        ...
        if self.PredatorObservation(state):
            ...
            allAgentLocations = [COORD(x, y) for x in range(self.XSize) for y in range(self.YSize)]
            invisibleAgentLocations = [coord for coord in allAgentLocations if self.Grid.VisualRay(coord, predLocation, self.Occlusions)]
            #validAgentLocations = list(set(allAgentLocations) - set(self.Occlusions))
            state.PredatorBeliefState = invisibleAgentLocations
    ...
    def Step(self, state, action):
        ...
        predatorObservation = self.PredatorObservation(state)
        if not predatorObservation:
    ...
    def PredatorAgentPosPropogation(self, state):
        ...
        allPossibleAgentNewPositions = [copyState.AgentPos]
        for n1 in range(N1):
    ...
    def MovePredator(self, state, move, previousPredatorLocation):
        ...
        for i in range(numberOfMoves):
            if Bernoulli(self.ChaseProbability):
                ...
                believedState = self.MovePredatorRandom(believedState, previousPredatorLocation)
    ...
    def MovePredatorRandom(self, state, previousPredatorLocation):
        ...
        while True:
            ...
            if self.Valid(newpos):  # and action != Opposite(state.PredatorDir):
                break
